...
If you see such information when you run `git status` command, it means that you are not on any branch but on some previous old version (the hash is the hash of commit to which your local repo is detached). If you have commited your changes and in this state you want to push them, run the following commands:
```
